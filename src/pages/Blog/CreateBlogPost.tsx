...
export default function CreateBlogPost() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitError(error.message || intl.formatMessage({ id: 'blog.create.error', defaultMessage: 'Failed to create blog post' }));
    }
  };
  ...
  const handleChange = (field: keyof CreateBlogPostRequest, value: string | boolean | null) => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const error = err as Error;
      setSubmitError(error.message || intl.formatMessage({ id: 'blog.create.uploadError', defaultMessage: 'Failed to upload image' }));
    }
    ...
    <div className="w-full h-[calc(100vh-65px)] absolute top-[65px] left-0 right-0 p-8 overflow-auto">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-6">
          <FormattedMessage id="blog.create.title" defaultMessage="Create Blog Post" />
        </h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 w-full">
          ...
          {(error || submitError) && (
            <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
              <p className="text-sm text-red-800 dark:text-red-200">
                {submitError || error?.message || intl.formatMessage({ id: 'blog.create.error', defaultMessage: 'Failed to create blog post' })}
              </p>
            </div>
          )}
          ...
          {uploadError && !uploading && (
            <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
              <p className="text-sm text-red-800 dark:text-red-200">
                {uploadError.message || intl.formatMessage({ id: 'blog.create.uploadError', defaultMessage: 'Failed to upload image' })}
              </p>
            </div>
          )}
            ...
            >
              {loading ? (
                <>
                  ...
                  <FormattedMessage id="blog.create.saving" defaultMessage="Saving..." />
                </>
              ) : (
                <>
                  <Save className="w-5 h-5 mr-2" />
                  <FormattedMessage id="blog.create.submit" defaultMessage="Create Post" />
                </>
              )}
            </Button>
            <Button
