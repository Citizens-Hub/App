...
export default function ShipNode({ data, id }: ShipNodeProps) {
  const { ship, onUpdateEdge, incomingEdges = [] } = data;
  ...
  const handleCustomPriceChange = (edgeId: string, price: number) => {
    setEdgeSettings(prevSettings => {
      // 获取当前的边缘设置
      const currentEdgeSettings = prevSettings[edgeId] || {};
  ...
  return (
    <div className="bg-gray-800 border border-gray-700 rounded-lg p-4 w-64 shadow-lg">
      ...
      <div className="flex flex-col items-center">
        <img 
          ...
          alt={ship.name}
          className="w-full h-28 object-cover rounded-md mb-12"
        />
        ...
        <div className="bg-blue-900 text-white font-bold py-1 px-3 rounded text-lg">
              ...
              {(edgeSettings[edge.id]?.sourceType === CcuSourceType.OFFICIAL_WB || 
                ...
                <div className="mb-2">
                  ...
                  <input 
                    ...
                    onChange={(e) => handleCustomPriceChange(edge.id, Number(e.target.value))}
